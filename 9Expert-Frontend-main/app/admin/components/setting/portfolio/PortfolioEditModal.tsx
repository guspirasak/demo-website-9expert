...
import { updatePortfolio } from "@/libs/AdminAPI"
import { imageToBase64, TImageToBase64Result } from "@/libs/ImageToBase64"
import { CloseIcon, EditIcon } from "@chakra-ui/icons"
import { Button, Divider, Heading, IconButton, Image, Input, MenuItem, Modal, ModalBody, ModalCloseButton, ModalContent, ModalHeader, ModalOverlay, Stack, Text, useDisclosure, useToast } from "@chakra-ui/react"
import { filesize } from "filesize"
import { useRef, useState } from "react"
import { Loading } from "../../Loading"

export const PortfolioEditModal = ({ portfolio, setUpdate }: { portfolio: TPortfolio, setUpdate: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const onHandleSave = async () => {
        setLoading(true)
        await updatePortfolio(state._id as string, state, (data: TCreatePortfolio, error: string) => {
            if (error) {
                console.log(error)
                setLoading(false)
                return toast({
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {
                                            state.file.name || state.imageUrl &&
                                            <AdminImageIcon
                                                w='30px'
                                                h='35px'
                                                color='#36B37E'
