...
import { useToast, useDisclosure, HStack, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, Stack, Input, Text, Image, Divider, Textarea, AspectRatio, Wrap, Center, Popover, PopoverTrigger, PopoverContent,  PopoverArrow, PopoverBody, PopoverCloseButton, PopoverFooter } from "@chakra-ui/react"
import { useState, useRef, useEffect } from "react"
import { Loading } from "../../../Loading"
import { TCreateCourseGroup, TCreateTechnologyArea, } from "@/app/admin/interface/TechnologyAreaInterface"
import { adminCreateCourseGroup, getTechnologyAreaById } from "@/libs/AdminAPI"
import ColorPicker from 'react-best-gradient-color-picker'
import rgbHex from "rgb-hex"

export const CGModal = ({ areaId, setUpdate }: { areaId: string, setUpdate: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const onHandleSave = async () => {
        setLoading(true)
        await adminCreateCourseGroup(areaId, state, (data: TCreateCourseGroup, error: string) => {

            if (error) {
                console.log(error)
                setLoading(false)
                return toast({
                    title: 'สร้างกลุ่มหลักสูตรไม่สําเร็จ',
                    status: 'error',
                    duration: 3000,
                    isClosable: true,
                    position: 'top-right'
            ...
            setUpdate(prev => !prev)
            toast({
                title: 'สร้างกลุ่มหลักสูตรสําเร็จ',
                status: 'success',
                duration: 3000,
                isClosable: true,
                position: 'top-right'
