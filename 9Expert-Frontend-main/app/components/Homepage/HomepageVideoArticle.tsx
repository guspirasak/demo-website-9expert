...
import { HomepageVideoArticleCard } from "../ContentCard/HomepageVideoArticleCard"
...
import { TTechnologyArea, TCourseGroup } from "@/app/admin/interface/CreateCourseInterface"
...
export const HomepageVideoArticle = ({ articles }: { articles: TArticle[] }) => {

    const [cache, setCache] = useState<TArticle[]>([])
    ...
    useEffect(() => {
        if (filter.technologyArea.length === 0 || filter.technologyArea.includes('All')) {
            ...
            getAllArticlesByMultiAreaName(filter.technologyArea, 'Video', (data: TArticle[], error: unknown) => {
    ...
    useEffect(() => {
        if (filter.selectedCourseGroup.length === 0 || filter.selectedCourseGroup.includes('All')) {
            ...
            getAllArticlesByMultiCourseGroupName(filter.selectedCourseGroup.map((item: string) => item), 'Video', (data: TArticle[], error: unknown) => {
                if (error) console.log(error)
    ...
    return (
        ...
        >
            <HStack
                ...
                spacing='2rem'
            >
                <Heading as='h2' size={{ base: 'lg', md: '2xl' }} >{`บทความวีดีโอ`}</Heading>
            </HStack>
                            ...
                            >
                                {cache.map((a, index) => (
                                    ...
                                    >
                                        <Center
                                            as={Link}
                                            href={`/article/${a.articleTitle.replaceAll(' ', '_')}`}
                                        >
                                            <HomepageVideoArticleCard article={a} />
                                        </Center>
                                    </SwiperSlide>
                                ))}
                                {
